...
import {
  ...
  createPlan, updatePlanStep, getPlan, listPlans,
  createDevNote, updateDevNote, listDevNotes,
  createCodeReview, updateCodeReview, listCodeReviews,
...
server.tool(
  'codebase_overview',
  'Get a high-level overview of the BackerHub codebase: file counts, directory structure, tech stack, lines of code.',
  {},
  async () => {
...
server.tool(
  'find_code',
  'Search the codebase for a pattern using ripgrep. Find usages, definitions, imports, etc.',
  {
  ...
  async ({ pattern, fileGlob, directory, contextLines }) => {
    ...
    if (!result) return { content: [{ type: 'text', text: `No matches found for "${pattern}"` }] };
...
server.tool(
  'list_api_routes',
  'List all API routes with their HTTP methods and a brief description of what each does.',
  {},
  async () => {
    const apiDir = resolve(PROJECT_ROOT, 'src/app/api');
    if (!existsSync(apiDir)) return { content: [{ type: 'text', text: 'No API routes found.' }] };
...
server.tool(
  'db_schema',
  'Show the database schema from Supabase migrations — tables, columns, functions, indexes.',
  {
  ...
  async ({ migration }) => {
    ...
    if (!existsSync(migDir)) return { content: [{ type: 'text', text: 'No migrations directory found.' }] };

    const files = readdirSync(migDir).filter(f => f.endsWith('.sql')).sort();

    if (migration) {
      const match = files.find(f => f.includes(migration));
      if (!match) return { content: [{ type: 'text', text: `Migration "${migration}" not found. Available: ${files.join(', ')}` }] };
    ...
    for (const file of files) {
      ...
      output += `## ${file}\n`;
      if (tables.length) output += `  Tables: ${tables.join(', ')}\n`;
      if (alters.length) output += `  Alters: ${[...new Set(alters)].join(', ')}\n`;
      if (functions.length) output += `  Functions: ${functions.join(', ')}\n`;
      output += '\n';
    }
...
server.tool(
  'component_tree',
  'Map out the component hierarchy and imports for a given component or page.',
  {
  ...
  async ({ path }) => {
    ...
    if (!existsSync(fullPath)) return { content: [{ type: 'text', text: `File not found: ${path}` }] };
    ...
    for (const m of localImports) output += `  ${m[1]}\n`;

    output += `\n## External Imports (${externalImports.length})\n`;
    for (const m of externalImports) output += `  ${m[1]}\n`;
    ...
    switch (check) {
      case 'outdated':
        ...
        try {
          ...
          const entries = Object.entries(parsed).map(([name, info]: [string, any]) =>
...
server.tool(
  'run_build',
  'Run the Next.js build and report any errors. Use to verify code compiles.',
  {
...
server.tool(
  'bundle_analysis',
  'Analyze the Next.js build output for bundle size and page sizes.',
  {},
  async () => {
...
server.tool(
  'check_env',
  'Check which environment variables are configured vs. required. Detect missing config.',
  {},
  async () => {
    // Read .env.local
    ...
    if (existsSync(envPath)) {
      readFileSync(envPath, 'utf-8').split('\n').forEach(line => {
        ...
        if (match) envVars[match[1]] = match[2] ? 'SET' : 'EMPTY';
      });
    }
...
server.tool(
  'git_status',
  'Show current git status: branch, uncommitted changes, recent commits.',
  {},
  async () => {
...
server.tool(
  'git_diff',
  'Show git diff for staged, unstaged, or between branches.',
  {
  ...
  async ({ target, branch, file }) => {
    ...
    switch (target) {
      ...
      case 'branch': cmd = `git diff ${branch || 'main'}...HEAD --stat`; break;
    }
    if (file) cmd = cmd.replace('--stat', '') + ` -- ${file}`;
...
server.tool(
  'generate_changelog',
  'Generate a changelog from git commits between two refs.',
  {
  ...
  async ({ from, to }) => {
    ...
    if (!log) return { content: [{ type: 'text', text: 'No commits found in range.' }] };
    ...
    for (const line of lines) {
      ...
      if (/^(feat|add|implement|create|build|integrate)/i.test(msg)) features.push(msg);
      else if (/^(fix|bug|patch|resolve|correct)/i.test(msg)) fixes.push(msg);
      else other.push(msg);
    }
...
server.tool(
  'create_impl_plan',
  'Create a detailed implementation plan for a feature — files to change, steps, risks, test strategy.',
  {
...
server.tool(
  'update_plan_step',
  'Mark an implementation plan step as done.',
  {
    planId: z.string(),
  ...
  async ({ planId, stepIndex, done }) => {
    ...
    if (!plan) return { content: [{ type: 'text', text: 'Plan or step not found.' }] };
...
server.tool(
  ...
  async ({ storyId }) => {
    ...
    if (!plans.length) return { content: [{ type: 'text', text: 'No implementation plans yet.' }] };
...
server.tool(
  'track_issue',
  'Track a dev issue: tech debt, security concern, performance problem, dependency risk, etc.',
  {
...
server.tool(
  'resolve_issue',
  'Mark a tracked dev issue as resolved with a resolution note.',
  {
    id: z.string(),
  ...
  async ({ id, resolution }) => {
    ...
    if (!note) return { content: [{ type: 'text', text: `Issue ${id} not found.` }] };
...
server.tool(
  'list_issues',
  'List tracked dev issues filtered by category, severity, or status.',
  {
  ...
  async (filters) => {
    ...
    if (!notes.length) return { content: [{ type: 'text', text: 'No issues found.' }] };
    const summary = notes.map(n =>
      `[${n.id}] ${n.severity.toUpperCase().padEnd(8)} ${n.status.padEnd(12)} ${n.category.padEnd(12)} ${n.title}`
    ).join('\n');
...
server.tool(
  'code_review',
  'Create a structured code review with findings categorized by type and severity.',
  {
    title: z.string(),
...
server.tool(
  'create_story',
  'Create a backlog item (feature, bug, tech-debt, improvement, research).',
  {
    title: z.string(),
    description: z.string(),
...
server.tool(
  ...
  async ({ id, ...updates }) => {
    ...
    if (!item) return { content: [{ type: 'text', text: `${id} not found.` }] };
...
server.tool(
  ...
  async (filters) => {
    ...
    if (!items.length) return { content: [{ type: 'text', text: 'Backlog empty.' }] };
    const summary = items.map(i =>
      `[${i.id}] ${i.priority.toUpperCase().padEnd(8)} ${i.status.padEnd(12)} ${i.title}`
    ).join('\n');
...
server.tool(
  ...
  async () => {
    ...
    if (stats.total > 0) {
      output += `## By Status\n`;
      for (const [s, c] of Object.entries(stats.byStatus)) output += `  ${s}: ${c}\n`;
    }
