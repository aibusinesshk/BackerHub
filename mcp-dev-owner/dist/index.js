...
import { initStore, createItem, updateItem, getItem, listItems, deleteItem, createSprint, addToSprint, updateSprint, getSprint, listSprints, getSprintBoard, createDecision, updateDecision, listDecisions, createMilestone, updateMilestone, listMilestones, getProjectStats, getData, } from './store.js';
// ─── Initialize ──────────────────────────────────────────────────────
...
// ─── PRODUCT OWNER TOOLS ─────────────────────────────────────────────
server.tool('create_story', 'Create a new user story / feature / bug / task in the backlog. Use this when planning new work.', {
...
server.tool('update_story', 'Update an existing backlog item (change status, priority, description, etc.)', {
...
server.tool('get_story', 'Get full details of a specific backlog item', { id: z.string().describe('Item ID') }, async ({ id }) => {
    const item = getItem(id);
    if (!item)
        return { content: [{ type: 'text', text: `Item ${id} not found.` }] };
    return { content: [{ type: 'text', text: fmt(item) }] };
});
server.tool('list_backlog', 'List and filter backlog items. Use to review what needs to be done, find bugs, check priorities.', {
    status: z.enum(['backlog', 'planned', 'in-progress', 'review', 'done', 'cancelled']).optional(),
    ...
    const summary = items.map((i) => `[${i.id}] ${i.priority.toUpperCase().padEnd(8)} ${i.status.padEnd(12)} ${i.type.padEnd(12)} ${i.title}${i.storyPoints ? ` (${i.storyPoints}pts)` : ''}`).join('\n');
    return {
        content: [{
                type: 'text',
...
server.tool('delete_story', 'Delete a backlog item permanently', { id: z.string().describe('Item ID to delete') }, async ({ id }) => {
    const ok = deleteItem(id);
    return { content: [{ type: 'text', text: ok ? `Deleted ${id}` : `Item ${id} not found.` }] };
});
server.tool('prioritize', 'Bulk-set priorities for multiple items at once. Use for backlog grooming.', {
...
// ─── SPRINT TOOLS ────────────────────────────────────────────────────
server.tool('create_sprint', 'Create a new sprint for organizing work into time-boxed iterations.', {
...
server.tool('add_to_sprint', 'Add a backlog item to a sprint', {
...
server.tool('sprint_board', 'View the sprint board showing items grouped by status (like a Kanban board). Great for standups.', {
    sprintId: z.string().describe('Sprint ID'),
}, async ({ sprintId }) => {
    const sprint = getSprint(sprintId);
    if (!sprint)
        return { content: [{ type: 'text', text: `Sprint ${sprintId} not found.` }] };
    const board = getSprintBoard(sprintId);
    const columns = ['planned', 'in-progress', 'review', 'done'];
    let output = `# Sprint: ${sprint.name}\nGoal: ${sprint.goal}\nStatus: ${sprint.status}\n`;
    if (sprint.startDate)
        output += `Period: ${sprint.startDate} → ${sprint.endDate || '?'}\n`;
    output += '\n';
...
server.tool('update_sprint', 'Update sprint status or details (start it, complete it, add retrospective)', {
    ...
    if (!sprint)
        return { content: [{ type: 'text', text: `Sprint ${id} not found.` }] };
    return { content: [{ type: 'text', text: `Updated sprint ${sprint.id}: status=${sprint.status}\n\n${fmt(sprint)}` }] };
});
server.tool('list_sprints', 'List all sprints', {}, async () => {
    const sprints = listSprints();
    if (sprints.length === 0)
        return { content: [{ type: 'text', text: 'No sprints yet. Use create_sprint to start one.' }] };
    const summary = sprints.map((s) => `[${s.id}] ${s.status.padEnd(10)} ${s.name} (${s.items.length} items) - ${s.goal}`).join('\n');
    return { content: [{ type: 'text', text: `${sprints.length} sprints:\n\n${summary}` }] };
});
// ─── ARCHITECTURE DECISION TOOLS ─────────────────────────────────────
server.tool('create_adr', 'Record an Architecture Decision Record (ADR). Use when making significant technical decisions.', {
...
server.tool('update_adr', 'Update an architecture decision (accept, deprecate, supersede)', {
...
server.tool('list_adrs', 'List all architecture decisions', {}, async () => {
    const decisions = listDecisions();
    if (decisions.length === 0)
        return { content: [{ type: 'text', text: 'No ADRs yet.' }] };
    const summary = decisions.map((d) => `[${d.id}] ${d.status.padEnd(12)} ${d.title}\n  Decision: ${d.decision.substring(0, 100)}...`).join('\n\n');
    return { content: [{ type: 'text', text: `${decisions.length} Architecture Decisions:\n\n${summary}` }] };
});
// ─── ROADMAP TOOLS ───────────────────────────────────────────────────
server.tool('create_milestone', 'Create a roadmap milestone to organize features into high-level release goals.', {
...
server.tool('update_milestone', 'Update a roadmap milestone', {
    ...
    if (!ms)
        return { content: [{ type: 'text', text: `Milestone ${id} not found.` }] };
    return { content: [{ type: 'text', text: `Updated milestone ${ms.id}\n\n${fmt(ms)}` }] };
});
server.tool('roadmap', 'View the full product roadmap with all milestones and their progress.', {}, async () => {
...
// ─── PROJECT ANALYTICS ───────────────────────────────────────────────
server.tool('project_stats', 'Get project analytics: velocity, item counts by status/type/priority, completion rate.', {}, async () => {
...
server.tool('standup', 'Generate a daily standup summary: what was done recently, what is in progress, and what is blocked.', {}, async () => {
    ...
    if (recentlyDone.length === 0)
        output += '  (nothing completed recently)\n';
    for (const i of recentlyDone)
        output += `  [${i.id}] ${i.title}\n`;
    output += `\n## In Progress - ${inProgress.length}\n`;
    if (inProgress.length === 0)
        output += '  (nothing in progress)\n';
    for (const i of inProgress)
        output += `  [${i.id}] ${i.priority.toUpperCase()} - ${i.title}\n`;
    output += `\n## In Review - ${inReview.length}\n`;
    if (inReview.length === 0)
        output += '  (nothing in review)\n';
    for (const i of inReview)
        output += `  [${i.id}] ${i.title}\n`;
    if (critical.length > 0) {
        output += `\n## CRITICAL Items - ${critical.length}\n`;
        for (const i of critical)
            output += `  [${i.id}] ${i.status} - ${i.title}\n`;
    }
...
// ─── RESOURCES ───────────────────────────────────────────────────────
server.resource('backlog', 'project://backlog', async (uri) => ({
    contents: [{
            uri: uri.href,
...
server.resource('roadmap', 'project://roadmap', async (uri) => ({
    contents: [{
            uri: uri.href,
            mimeType: 'application/json',
...
server.resource('decisions', 'project://decisions', async (uri) => ({
    contents: [{
            uri: uri.href,
            mimeType: 'application/json',
